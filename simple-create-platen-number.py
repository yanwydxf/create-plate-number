...
# 保存车牌图片和记录生成后车牌号文本
def wirte_imglabel(platennumber, font, label):
    # 如果保存车牌目录不存在则进行创建
...
# 开始生成
for x in range(size):
    platennumber = random_platen()  # 得到随机组成的车牌号
    font = pygame.font.Font(os.path.join(filepath, "simhei.ttf"), 32)
    fobj = font.render(platennumber, True, (0, 0, 0), (255, 255, 255))
    # 记录所生成的车牌号
    label = platennumber+".jpg" + ","
    # 生成
    wirte_imglabel(platennumber, fobj, label)
    print(platennumber)
